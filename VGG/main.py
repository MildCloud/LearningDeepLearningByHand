...
def vgg_block(f_num_convolutions, f_in_channels, f_out_channels):
    layers = []
    for _ in range(f_num_convolutions):
        layers.append(nn.Conv2d(f_in_channels, f_out_channels, (3, 3), padding=1))
        layers.append(nn.ReLU())
        f_in_channels = f_out_channels
...
def vgg(f_conv_arch):
    conv_blocks = []
    f_in_channels = 1
    for (f_num_convolutions, f_out_channels) in f_conv_arch:
        conv_blocks.append(vgg_block(f_num_convolutions, f_in_channels, f_out_channels))
        f_in_channels = f_out_channels
    return nn.Sequential(
        *conv_blocks, nn.Flatten(),
        nn.Linear(f_in_channels * 7 * 7, 4096), nn.ReLU(), nn.Dropout(0.5),
...
"""Construct a smaller network to train"""
ratio = 4
