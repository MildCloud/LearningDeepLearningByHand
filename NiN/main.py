...
def nin_block(f_in_channels, f_out_channels, f_kernel_size, f_strides, f_padding):
    return nn.Sequential(
        nn.Conv2d(f_in_channels, f_out_channels, f_kernel_size, f_strides, f_padding),
        nn.ReLU(),
        nn.Conv2d(f_out_channels, f_out_channels, (1, 1)),
        nn.ReLU(),
        nn.Conv2d(f_out_channels, f_out_channels, (1, 1)),
        nn.ReLU()
    )
...
net = nn.Sequential(
    nin_block(1, 96, (11, 11), 4, 0),
    nn.MaxPool2d(3, stride=2),
    nin_block(96, 256, (5, 5), 1, 2),
    nn.MaxPool2d(3, stride=2),
    nin_block(256, 384, (3, 3), 1, 1),
    nn.MaxPool2d(3, stride=2),
    nn.Dropout(0.5),
    nin_block(384, 10, (3, 3), 1, 1),
    nn.AdaptiveAvgPool2d((1, 1)),
    # The AdaptiveAvg function will not change the number of channel and (1, 1) means the shape of output
    # (1, 1) means the the kernel size = H_in * W_in
    nn.Flatten()
)
